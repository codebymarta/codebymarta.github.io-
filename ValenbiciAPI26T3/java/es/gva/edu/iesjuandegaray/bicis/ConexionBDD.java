...
public class ConexionBDD extends JFrame {
    ...
    public ConexionBDD() {
     ...
     // Lógica para añadir los datos a MySQL
        btnAnadir.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (con == null || dJSon == null) {
                    txtPrimeroObtenerDatos.setText("Faltan datos o conexión");
                    return;
                }
                try {
                    s = con.createStatement();
                    for (String linea : dJSon.getValues()) {
                        if (linea != null) {
                            String[] v = linea.split(";");
                            
                            // LIMPIEZA: Quitamos comillas del nombre de la calle para que no rompa el SQL
                            String direccionLimpia = v[1].replace("'", " "); 
                            
                            // SQL DEFINITIVO
                            String sql = "INSERT INTO historico (estacion_id, direccion, bicis_disponibles, anclajes_libres, estado_operativo, ubicacion) " +
                                         "VALUES (" + v[0] + ", '" + direccionLimpia + "', " + v[2] + ", " + v[3] + ", 1, ST_GeomFromText('POINT(" + v[4] + " " + v[5] + ")'))";
                            
                            s.executeUpdate(sql);
                        }
                    }
                    txtPrimeroObtenerDatos.setText("¡POR FIN! Guardado con éxito");
                } catch (SQLException ex) {
                    // Si falla, vamos a imprimir el error exacto en la consola de Eclipse para verlo
                    txtPrimeroObtenerDatos.setText("Error: " + ex.getMessage());
                    System.out.println("SQL que ha fallado: " + ex.getMessage());
                }
            }
        });
